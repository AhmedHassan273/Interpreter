...
void Interpreter::Evaluate_expressions()
{
...
bool Interpreter::has_Values()
{
	return index < values.size();
}

Value * Interpreter::NextVal()
{
...
Value * Interpreter::Evaluate(Exp * exp, Scope *scope)
{
	...
	if (exp_type == IF)
	{
		...
		if (conditionValue->Val)
		{
			for (int i = 0; i < ifelse->thenExp.size(); i++)
				values.push_back(Evaluate(ifelse->thenExp[i], ifscope));
		}
		else
		{
			for (int i = 0; i < ifelse->elseExp.size(); i++)
				values.push_back(Evaluate(ifelse->elseExp[i], elsescope));
		}
	}
	if (exp_type == While_exp)
	{
		...
		while (conditionValue->Val)
		{
			for (int i = 0; i < while_loop->exps.size(); i++)
				values.push_back(Evaluate(while_loop->exps[i], whilescope));

			condition = Evaluate(while_loop->conditionExp, whilescope);
...
Value * Interpreter::DoBiOperation(string operation, Value *op1, Value *op2)
{
	if (operation == "+")
	{
